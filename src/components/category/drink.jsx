...
const Drink = ({ user: propUser, setShowLogin }) => {
  ...
  // Lấy sản phẩm từ server, chỉ lấy category === "đồ uống"
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_API_URL}/api/products`)
      .then(res => {
        if (res.data.success) {
          setProducts(res.data.products.filter(p => p.category === "đồ uống"));
        }
      })
      .catch(err => {
  ...
  // Lọc các loại (type) trong đồ uống
  ...
  const handleAddToCart = async () => {
    ...
    const unitPrice = basePrice + extrasPrice - (selectedProduct.discount || 0);
  ...
  const renderProducts = (title, products) => (
    <div key={title}>
      <h1 className="mt-4 mb-4 text-xl font-bold text-black">Đồ uống {title}</h1>
