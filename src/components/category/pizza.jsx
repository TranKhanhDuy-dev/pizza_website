...
const Pizza = ({ user: propUser, type: propType, setShowLogin }) => {
  ...
  const renderProducts = (title, products) => (
    <div key={title}>
      <h1 className="mt-4 mb-4 text-xl font-bold text-black">Pizza {title}</h1>
      <div className="flex max-w-full gap-4 overflow-x-auto sm:grid sm:grid-cols-5 sm:overflow-x-hidden">
        {products.map((product, index) => (
          ...
          >
            ...
            />
            <p className="text-base font-semibold text-gray-900 h-[48px] line-clamp-2 break-words text-center">
              {product.productName}
            </p>
