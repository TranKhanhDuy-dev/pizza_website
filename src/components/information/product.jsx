...
const Product = ({ user, type, setShowLogin }) => {
  ...
  const recalculatePrice = (base, sizeName, crustName, extras, qty) => {
    const sizePrice = selectedProduct.size.find(s => s.name === sizeName)?.price || 0;
    const crustPrice = selectedProduct.crust.find(c => c.name === crustName)?.price || 0;
    ...
    return (base + sizePrice + crustPrice + extrasPrice) * qty;
  };
  ...
  return (
    <div className="w-full pt-4 bg-white">
      <h1 className="mb-4 text-xl font-bold text-black">Sản phẩm</h1>
      ...
      <div className="flex justify-center mt-6">
        <button
          className="px-6 py-2 text-white transition bg-[#e50914] rounded hover:bg-red-500"
