...
const AllCombo = ({ user, setShowLogin }) => {
    ...
    axios.get(`${import.meta.env.VITE_API_URL}/api/products`)
      .then(res => {
        if (res.data.success) {
          ...
          setProducts(res.data.products.filter(p => (p.category || "").toLowerCase() === "combo"));
        }
      })
      .catch(err => {
  ...
  const renderProducts = (products) => (
    <div>
      <h1 className="mt-4 mb-4 text-xl font-bold text-black">Combo</h1>
  ...
  return (
    <div className="flex flex-grow w-full pb-4 bg-white md:pl-24">
      ...
      <div className="w-full md:w-[72%] pt-6">
        {/* Chỉ hiện danh mục "Tất cả" */}
        <div className="flex flex-wrap items-center gap-2 mb-6">
          <button
            className="px-4 py-2 text-sm font-medium rounded-full border bg-[#e50914] text-white"
            disabled
          >
            Tất cả
          </button>
