...
const Find = ({ user: propUser, type, setShowLogin }) => {
  ...
  useEffect(() => {
    if (selectedProduct) {
      ...
      let price = selectedProduct.quantity + sizePrice + crustPrice + extrasPrice;
  ...
  return (
    <div className="flex flex-grow w-full pb-4 pl-24 bg-white">
        <Category />
      <div className="w-full md:w-[72%] pt-10">
        ...
        {loading ? (
          ...
          <div className="flex flex-wrap gap-4">
            {products.map(product => (
              <div
                key={product._id}
                className="relative flex flex-col items-center w-48 p-3 transition-shadow bg-gray-100 rounded-lg shadow-md cursor-pointer group"
              ...
              >
                ...
                />
                <p className="text-base font-semibold text-gray-900 h-[48px] line-clamp-2 break-words text-center">{product.productName}</p>
                <div className="flex items-center justify-between w-full mt-2">
                  {product.discount > 0 ? (
                    <div>
                      <span className="text-xs text-gray-500 line-through">{product.quantity.toLocaleString()}đ</span>
                      <span className="ml-1 text-base font-bold text-red-600">
